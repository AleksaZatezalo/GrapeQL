...
class TestInfoTester:
    ...
    async def test_introspection_enabled(
        self, info_tester, mock_aioresponses, sample_graphql_schema
    ):
        ...
        introspection_finding = next(
            (f for f in findings if "introspection" in str(f).lower()), None
        )
    ...
    async def test_field_suggestions_detection(self, info_tester, mock_aioresponses):
        ...
        suggestion_finding = next(
            (
                f
                for f in findings
                if "suggestion" in str(f).lower() or "did you mean" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_schema_leak_detection(self, info_tester, mock_aioresponses):
        ...
        stacktrace_finding = next(
            (
                f
                for f in findings
                if "stacktrace" in str(f).lower() or "exception" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_debug_mode_detection(self, info_tester, mock_aioresponses):
                ...
                "errors": [
                    {
                        ...
                        "extensions": {
                            "exception": {"stacktrace": ["Full stack trace here..."]}
                        },
                    }
                ]
            },
        ...
        debug_finding = next(
            (
                f
                for f in findings
                if "debug" in str(f).lower() or "stacktrace" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_graphql_playground_detection(self, info_tester, mock_aioresponses):
        ...
        playground_finding = next(
            (f for f in findings if "playground" in str(f).lower()), None
        )
    ...
    async def test_error_message_analysis(self, info_tester, mock_aioresponses):
        ...
        db_info_finding = next(
            (
                f
                for f in findings
                if "database" in str(f).lower() or "connection" in str(f).lower()
            ),
            None,
        )
    ...
    def test_get_findings(self, info_tester):
        ...
        info_tester.findings = [
            ...
            {
                "type": "medium",
                "message": "Field suggestions revealed in error messages",
            },
