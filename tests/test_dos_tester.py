...
class TestDosTester:
    ...
    async def test_circular_query_detection(self, dos_tester, mock_aioresponses):
        ...
        mock_aioresponses.post(
            endpoint,
            payload={"data": {"user": {"friends": [{"friends": [{"friends": []}]}]}}},
            status=200,
        )
        ...
        circular_finding = next(
            (
                f
                for f in findings
                if "circular" in str(f).lower() or "recursive" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_deeply_nested_query_detection(self, dos_tester, mock_aioresponses):
        ...
        nested_finding = next(
            (
                f
                for f in findings
                if "nested" in str(f).lower() or "depth" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_alias_overloading_detection(self, dos_tester, mock_aioresponses):
        ...
        alias_finding = next(
            (
                f
                for f in findings
                if "alias" in str(f).lower() or "overload" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_field_duplication_detection(self, dos_tester, mock_aioresponses):
        ...
        mock_aioresponses.post(
            endpoint,
            payload={"data": {"user": {"id": "1", "username": "test"}}},
            status=200,
        )
        ...
        duplication_finding = next(
            (
                f
                for f in findings
                if "duplicate" in str(f).lower() or "repeated" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_array_based_dos_detection(self, dos_tester, mock_aioresponses):
        ...
        mock_aioresponses.post(
            endpoint,
            payload={"data": {"users": [{"id": str(i)} for i in range(1000)]}},
            status=200,
        )
        ...
        array_finding = next(
            (
                f
                for f in findings
                if "array" in str(f).lower() or "list" in str(f).lower()
            ),
            None,
        )
    ...
    async def test_timeout_detection(self, dos_tester, mock_aioresponses):
        ...
        mock_aioresponses.post(
            endpoint, exception=asyncio.TimeoutError("Query timeout")
        )
        ...
        timeout_finding = next(
            (f for f in findings if "timeout" in str(f).lower()), None
        )
    ...
    async def test_complexity_analysis(self, dos_tester, mock_aioresponses):
        ...
        complexity_finding = next(
            (f for f in findings if "complexity" in str(f).lower()), None
        )
    ...
    async def test_rate_limiting_detection(self, dos_tester, mock_aioresponses):
        ...
        rate_limit_finding = next(
            (
                f
                for f in findings
                if "rate" in str(f).lower() or "limit" in str(f).lower()
            ),
            None,
        )
