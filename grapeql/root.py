...
class root():
    ...
    async def runIntrospection(self, session: aiohttp.ClientSession) -> bool:
        ...
        try:
            async with session.post(
                ...
                self.message.printMsg("Introspection failed - endpoint might not be GraphQL", status="error")
                return False
    ...
    async def fingerprintEngine(self) -> Optional[str]:
        ...
        if not self.endpoint:
            self.message.printMsg("No endpoint set. Call setEndpoint first.", status="error")
            return None
        ...
        try:
            ...
            self.message.printMsg(f"Error during engine fingerprinting: {str(e)}", status="error")
            return None
    ...
    async def testYoga(self, session: aiohttp.ClientSession) -> bool:
        ...
        return (self._error_contains(response, 'asyncExecutionResult[Symbol.asyncIterator] is not a function') or 
    ...
    async def testApollo(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'Directive "@skip" argument "if" of type "Boolean!" is required'):
            return True

        query = "query @deprecated { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Directive "@deprecated" may not be used on QUERY')
    ...
    async def testHasura(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = "query { aaa }"
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'field "aaa" not found in type: \'query_root\''):
            return True
    ...
    async def testGraphqlPhp(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'Syntax Error: Cannot parse the unexpected character "?"'):
            return True
    ...
    async def testRuby(self, session: aiohttp.ClientSession) -> bool:
        ...
        if self._error_contains(response, '\'@skip\' can\'t be applied to queries'):
            return True
        elif self._error_contains(response, 'Directive \'skip\' is missing required arguments: if'):
            return True
    ...
    async def testHyperGraphql(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'Validation error of type InvalidSyntax: Invalid query syntax.'):
            return True

        query = "query { alias1:__typename @deprecated }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Validation error of type UnknownDirective: Unknown directive deprecated')
    ...
    async def testGraphqlJava(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'Validation error of type DuplicateDirectiveName'):
            return True

        query = ""
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Invalid Syntax : offending token \'<EOF>\'')
    ...
    async def testAriadne(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'Unknown directive \'@abc\'.') and 'data' not in response:
            return True
    ...
    async def testWpGraphql(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = ""
        response = await self._graphql_request(session, query)
        if self._error_contains(response, 'GraphQL Request must include at least one of those two parameters: "query" or "queryId"'):
            return True
    ...
    async def testSangria(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = "queryy { __typename }"
        response = await self._graphql_request(session, query)
        return 'Syntax error while parsing GraphQL query. Invalid input "queryy"' in response.get('syntaxError', '')
    ...
    async def testFlutter(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Directive "deprecated" may not be used on FIELD.')

    async def testDianaJl(self, session: aiohttp.ClientSession) -> bool:
        """Test if the endpoint is running Diana.jl."""
        query = "queryy { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Syntax Error GraphQL request (1:1) Unexpected Name "queryy"')

    async def testStrawberry(self, session: aiohttp.ClientSession) -> bool:
        """Test if the endpoint is running Strawberry."""
        query = "query @deprecated { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Directive \'@deprecated\' may not be used on query.') and 'data' in response
    ...
    async def testTartiflette(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = "query @skip { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Missing mandatory argument < if > in directive < @skip >.')
    ...
    async def testDgraph(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = "query { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Not resolving __typename. There\'s no GraphQL schema in Dgraph.')
    ...
    async def testDirectus(self, session: aiohttp.ClientSession) -> bool:
        ...
        return errors and errors[0].get('extensions', {}).get('code') == 'INVALID_PAYLOAD'
    ...
    async def testLighthouse(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        return (self._error_contains(response, 'Internal server error') or 
    ...
    async def testMorpheus(self, session: aiohttp.ClientSession) -> bool:
        ...
        query = "queryy { __typename }"
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'expecting white space') or self._error_contains(response, 'offset')
    ...
    async def testLacinia(self, session: aiohttp.ClientSession) -> bool:
        ...
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Cannot query field `graphw00f\' on type `QueryRoot\'.')
    ...
    async def testCaliban(self, session: aiohttp.ClientSession) -> bool:
        ...
        """
        response = await self._graphql_request(session, query)
        return self._error_contains(response, 'Fragment \'woof\' is not used in any spread')


    async def _graphql_request(self, session: aiohttp.ClientSession, query: str, operation: str = None) -> Dict:
