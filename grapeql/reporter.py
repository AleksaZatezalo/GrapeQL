...
class Reporter:
    ...
    def print_summary(self) -> None:
        ...
        if severity_counts["CRITICAL"] > 0:
            print(f"  {self.printer.RED}{self.printer.BOLD}CRITICAL: {severity_counts['CRITICAL']}{self.printer.END}")
        
        if severity_counts["HIGH"] > 0:
            print(f"  {self.printer.RED}HIGH: {severity_counts['HIGH']}{self.printer.END}")
            
        if severity_counts["MEDIUM"] > 0:
            print(f"  {self.printer.YELLOW}MEDIUM: {severity_counts['MEDIUM']}{self.printer.END}")
            
        if severity_counts["LOW"] > 0:
            print(f"  {self.printer.BLUE}LOW: {severity_counts['LOW']}{self.printer.END}")
        ...
        if severity_counts["CRITICAL"] > 0 or severity_counts["HIGH"] > 0:
            ...
            for finding in self.findings:
                if finding.severity.upper() in ["CRITICAL", "HIGH"]:
                    ...
                    print(f"{color}{finding.severity}: {finding.title} - {finding.endpoint}{self.printer.END}")
...
"""
    ...
    def generate_json(self, output_file: str) -> None:
        ...
        try:
            ...
            self.printer.print_msg(f"JSON report written to {output_file}", status="success")
        except Exception as e:
            self.printer.print_msg(f"Error writing JSON report: {str(e)}", status="error")
                
    def generate_report(self, output_format: str = "markdown", output_file: str = None) -> None:
        """
        ...
        if output_format.lower() == "markdown" or output_format.lower() == "md":
            ...
            self.printer.print_msg(f"Unsupported report format: {output_format}", status="error")
