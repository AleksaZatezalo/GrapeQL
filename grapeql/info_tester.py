    ...
    async def run_test(self) -> List[Finding]:
        ...
        if not self.client.endpoint:
            self.printer.print_msg(
                "No endpoint set. Run setup_endpoint first.",
                status="error"
            )
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                self.printer.print_msg(f"Error testing {test_name}: {str(e)}", status="error")
                
        if not self.findings:
            self.printer.print_msg("No information disclosure issues found", status="success")
