...
class DosTester(VulnerabilityTester):
    ...
    def generate_circular_query(self) -> str:
        ...
        for type_name, type_info in self.types.items():
            for field in type_info.get("fields", []):
                ...
                target_type = field_type.get("name") or field_type.get(
                    "ofType", {}
                ).get("name")
    ...
    def generate_deeply_nested_query(self) -> str:
        ...
        for field in self.types[self.query_type].get("fields", []):
            field_type = field.get("type", {})
            target_type = field_type.get("name") or field_type.get("ofType", {}).get(
                "name"
            )
    ...
    def generate_fragment_bomb(self) -> str:
        ...
        for type_name, type_info in self.types.items():
            if (
                type_name != "Query"
                and type_name != "Mutation"
                and type_info.get("fields")
            ):
    ...
    async def test_query(self, query: str, timeout: float = 10.0) -> Tuple[bool, float]:
        ...
        try:
            ...
            is_vulnerable = duration > 5.0 or (
                response
                and "errors" in response
                and any(
                    "timeout" in str(err.get("message", "")).lower()
                    or "memory" in str(err.get("message", "")).lower()
                    or "stack" in str(err.get("message", "")).lower()
    ...
    async def run_test(self) -> List[Finding]:
        ...
        for test_name, query_generator in tests:
            ...
            if is_vulnerable:
                ...
                self.printer.print_msg(
                    f"Endpoint is NOT vulnerable to {test_name}", status="success"
                )
                self.printer.print_msg(
        ...
        if batch_queries:
            ...
            try:
                ...
                response, _ = await self.client.make_request("POST", json=batch_queries)
                ...
                if is_vulnerable:
                    self.printer.print_msg(
                        "Endpoint is VULNERABLE to Array Batching DoS!", status="failed"
                    )
                    self.printer.print_msg(
                        f"Response time: {duration:.2f} seconds", status="failed"
                    )
                ...
                self.printer.print_msg(
                    f"Error testing Array Batching: {str(e)}", status="error"
                )
        else:
            self.printer.print_msg(
