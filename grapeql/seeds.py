...
class seeds:
        ...
        try:
            ...
            if result.get("data", {}).get("__schema"):
                self.message.printMsg(
                    "Starting basic vulnerability scanning", status="success"
                )
                return True
    ...
    async def checkGetMethodSupport(self, session: aiohttp.ClientSession) -> Dict:
        ...
        try:
            ...
            response_data = await self.makeRequest(
                session, method="GET", params={"query": query}
            )
            res["curl_verify"] = self.generateCurl()
    ...
    async def checkPostBasedCsrf(self, session: aiohttp.ClientSession) -> Dict:
        ...
        try:
            ...
            response_data = await self.makeRequest(
                session, method="POST", data=form_data
            )
            res["curl_verify"] = self.generateCurl()
